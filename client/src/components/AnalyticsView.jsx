...
function AnalyticsView({ stats }) {
  const chartData = [
    ...
    { name: 'Contacted', value: stats.Contacted || stats.contacted || 0, color: '#f59e0b' },
    { name: 'Qualified', value: stats.Qualified || stats.qualified || 0, color: '#7c3aed' },
    { name: 'Converted', value: stats.Converted || stats.converted || 0, color: '#059669' },
  ...
  const activeFunnelCount = (stats.New || 0) + (stats.Contacted || 0) + (stats.Qualified || 0);
  const activeFunnelPercent = totalLeads ? ((activeFunnelCount / totalLeads) * 100).toFixed(0) : 0;
  const lossRatioPercent = totalLeads ? (((stats.Lost || 0) / totalLeads) * 100).toFixed(0) : 0;
          ...
          <div className="analytics-card mini-card">
            ...
            <div className="progress-group">
              ...
              <div className="progress-bar-bg">
                <div className="progress-bar-fill color-blue" style={{ width: `${activeFunnelPercent}%` }}></div>
              </div>
            </div>
            ...
            <div className="progress-group pipeline-margin-top">
              ...
              <div className="progress-bar-bg">
                <div className="progress-bar-fill color-red" style={{ width: `${lossRatioPercent}%` }}></div>
              </div>
            </div>
          </div>

          <div className="analytics-card mini-card updates-card">
            <div className="card-title-wrapper">
              <Info className="title-icon icon-blue" />
              <h4>Live Recharts Integration</h4>
            </div>
            <p className="info-desc">
              Backend API synchronization active.
            </p>
          </div>
        </div>
      </div>
